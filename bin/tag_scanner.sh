...
# 0. This script expects that there are nearby beacons using Bluetooth Low Energy (BLE) to send out data packets in advertising/broadcast mode.
#    This Bash script sits on a Raspberry Pi and expects that you've installed all the libaries it references, special for this project as well as generic. 
#	 This script combined with a ruby script captures the BLE data packets and sends the packets to Google Cloud.
#
...
#
# 3. Output the data from the BLE scanner using the command: `sudo hcidump -t --raw | aggregate_data_packet`
#    sudo = superuser
...
#    | aggregate_data_packet = pump the data into a function called aggregate_data_packet. "|" is for functions; if we used ">" instead, the data would go into a file with the name aggregate_data_packet
#
...
#    `aggregate_data_packet` is our function that removes the carriage returns and gathers all the data from a packet
#    (2) `hcidump --raw` outputs characters that aren't bytes: ">" and " " (carrots and whitespace)
#    `aggregate_data_packet` also removes those characters and simplifies the data packet to just bytes.
#
...
if [ "$(pidof hcitool)" ]; then
  # start dumping the output from BLE scanner (with timestamps -t), aggregate packets across line breaks, filter to only include Fujitsu and output in JSON format.
  start_hcidump_stream | aggregate_data_packet | filter_and_JSONify_fujitsu_packets
else
...
# Note 
# Jon originally pulled this Bash script for an iBeacon Scanner.  We reconfigured it for the DREAM project with Fujitsu beacons
